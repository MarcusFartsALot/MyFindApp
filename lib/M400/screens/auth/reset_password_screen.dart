...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await _authService.completePasswordRecovery(_passwordCtrl.text);
      if (!mounted) return;
  ...
  Future<void> _cancel() async {
    ...
    setState(() => _isSubmitting = true);
    try {
      await _authService.discardRecoverySession();
      if (mounted) _returnToLogin();
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Could not clear this session. Please close the app and sign in again.',
            ),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: M400AuthPage(
        ...
        subtitle:
            'Choose a strong password to keep your MyFind account secure.',
